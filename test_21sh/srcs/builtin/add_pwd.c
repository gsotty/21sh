...
/*   Updated: 2017/06/02 11:12:31 by gsotty           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "../../includes/builtin.h"

static void		verif_pwd_2(char *path, char *tmp_pwd, char ***envp)
{
	struct stat		lg;
	struct stat		phy;
	char			pwd[PATH_MAX + 1];
...
static void		verif_pwd(char *path, char *tmp_pwd, char ***envp)
{
	char			pwd[PATH_MAX + 1];

	if (path != NULL && *path == '/')
		verif_pwd_2(path, tmp_pwd, envp);
	else
	{
		getcwd(pwd, PATH_MAX);
		tmp_pwd = ft_strjoin("PWD=", pwd);
