...
/*   Updated: 2019/04/04 14:13:14 by gsotty           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int			ft_add_new_envp(int x, char *name_var, char *data_var,
		t_envp *my_envp)
{
	int		len_data;
	int		len_name;
	...
	if (data_var != NULL)
		...
		len_data = 0;
	if ((my_envp->envp[x] = ft_memalloc((sizeof(char *) *
						(len_name + len_data + 1)) + 1)) == NULL)
		return (1);
...
int			ft_replace_envp(int x, char *name_var, char *data_var,
		t_envp *my_envp)
{
	int		len_data;
	int		len_name;
...
int			envp_no_null(char *name_var, char *data_var, t_envp *my_envp)
{
	...
	{
		if ((ft_add_new_envp(x, name_var, data_var, my_envp)) == 1)
			return (1);
	}
	else
	{
		if ((ft_replace_envp(x, name_var, data_var, my_envp)) == 1)
			return (1);
	}
	return (0);
}
