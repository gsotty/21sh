...
int				ft_history_copy(t_history *history_first, t_history *history_copy)
{
	...
	history_copy->malloc_buf = history_first->malloc_buf;
	history_copy->pos_buf = history_first->pos_buf;
	if ((history_copy->len = ft_memalloc(sizeof(int) * (history_first->malloc_buf + 2))) == NULL)
		return (1);
	if ((history_copy->len_malloc = ft_memalloc(sizeof(int) * (history_first->malloc_buf + 2))) == NULL)
		return (1);
	if ((history_copy->pos = ft_memalloc(sizeof(int) * (history_first->malloc_buf + 2))) == NULL)
		return (1);
	if ((history_copy->buf = ft_memalloc(sizeof(char *) * (history_first->malloc_buf + 2))) == NULL)
		return (1);
	cont = 0;
	while (cont < (history_first->malloc_buf + 1))
	{
		if (history_first->buf[cont] != NULL)
		{
			...
			if ((history_copy->buf[cont] = ft_memalloc(sizeof(char) * (history_first->len_malloc[cont] + 1))) == NULL)
				return (1);
...
int				add_new_history(t_history *history_first, t_history *history_copy)
{
	...
	if (history_first->len_buf >= history_first->malloc_buf)
	{
		...
		if ((tmp_len = ft_memalloc(sizeof(int) * (history_first->malloc_buf + 2))) == NULL)
			return (1);
		if ((tmp_len_malloc = ft_memalloc(sizeof(int) * (history_first->malloc_buf + 2))) == NULL)
			return (1);
		if ((tmp_pos = ft_memalloc(sizeof(int) * (history_first->malloc_buf + 2))) == NULL)
			return (1);
		if ((tmp_buf = ft_memalloc(sizeof(char *) * (history_first->malloc_buf + 2))) == NULL)
			return (1);
		cont = 0;
		while (cont < history_first->malloc_buf)
		{
			if (history_first->buf[cont] != NULL)
			{
				...
				if ((tmp_buf[cont] = ft_memalloc(sizeof(char) * (history_first->len_malloc[cont] + 1))) == NULL)
					return (1);
