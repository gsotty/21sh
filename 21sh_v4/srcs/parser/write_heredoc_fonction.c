...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2017/09/28 13:08:04 by gsotty            #+#    #+#             */
/*   Updated: 2017/09/28 14:27:15 by gsotty           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int		len_of_nbr_ligne_2(struct winsize win, int len)
{
	...
	while (tmp_len > 0)
	{
		if (nbr_ligne_f == -1)
		{
			tmp_len -= (win.ws_col - _PROMPT_LEN_ML);
			nbr_ligne_f++;
		}
		else
		{
...
static int		verif_creat_ligne(struct winsize win, int nbr_ligne_f, int len)
{
	if (nbr_ligne_f > 0)
	{
		if (nbr_ligne_f == 1)
		{
			if ((len - (win.ws_col - _PROMPT_LEN_ML)) >= win.ws_col)
				return (1);
		}
		else
		{
	...
	else if (len >= (win.ws_col - _PROMPT_LEN_ML))
		return (1);
	return (0);
}
...
void			new_safe_place_heredoc(int len, int nbr_new_line)
{
	...
	if (verif_creat_ligne(win, nbr_ligne_f, len) == 0)
	{
		if (nbr_ligne_f > 0)
		{
			...
			tputs(tgoto(tgetstr("RI", NULL), 0, _PROMPT_LEN_ML), 0, f_putchar);
